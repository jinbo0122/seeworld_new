...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    if (_attributedString == nil) {
        ...
        CGFloat height = 0.0f;
        if (_currentNumberOfLines == kMaxNumberOfLines || _currentNumberOfLines > lineCount) {
            height = [drawString prepareDisplayViewHeightWithWidth:self.width];
        }
        else if (_currentNumberOfLines != lineCount) {
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (self.isSelected && NSLocationInRange(index, self.moreRange) && self.numberOfLines > 0) {
        ...
        CTFrameRef frameRef = [_currentAttString prepareFrameRefWithWidth:self.width];
        CFArrayRef lines = CTFrameGetLines(frameRef);
        CFIndex lineCount = CFArrayGetCount(lines);
        
        if (_currentNumberOfLines == _numberOfLines) {
            NSMutableAttributedString *closeDrawString = [[self parseAttributedWithAttributedString:_attributedString] mutableCopy];
            [closeDrawString appendAttributedString:_attributedCloseString];
            
            CTFrameRef closeFrameRef = [closeDrawString prepareFrameRefWithWidth:self.width];
            CFIndex closeLineCount = CFArrayGetCount(CTFrameGetLines(closeFrameRef));
            
            if (closeLineCount == lineCount) { // 能同行展示的时候
                self.frameRef = closeFrameRef;
                _currentAttString = [closeDrawString mutableCopy];
                _currentNumberOfLines = lineCount;
            }
            else { // 换行展示
                NSMutableAttributedString *drawString = [self createdrawAttributedString];
                self.currentAttString = [drawString mutableCopy];
                NSLog(@"self.currentAttString===%@", self.currentAttString.string);
            ...
            _currentNumberOfLines = _numberOfLines;
            _currentAttString = [_attributedString mutableCopy];
...
- (void)drawRect:(CGRect)rect {
    ...
    // 3.绘制
    NSMutableAttributedString *drawString = [self parseAttributedWithAttributedString:_attributedString];
    
    self.frameRef = [drawString prepareFrameRefWithWidth:self.width];
    CFIndex lineCount = CFArrayGetCount(CTFrameGetLines(self.frameRef));
    
    CTFrameRef closeFrameRef = [_currentAttString prepareFrameRefWithWidth:self.width];
    CFIndex closeLineCount = CFArrayGetCount(CTFrameGetLines(closeFrameRef));

    if (self.currentNumberOfLines == kMaxNumberOfLines || (self.currentNumberOfLines > lineCount && lineCount == closeLineCount)) {
        CTFrameDraw(self.frameRef, context);
    }
    else {
        if ([_currentAttString.string rangeOfString:_attributedCloseString.string].location != NSNotFound) {
            
            // 添加［添加收起］
            if (lineCount == closeLineCount) { // 当不用换行的时候
                self.frameRef = [_currentAttString prepareFrameRefWithWidth:self.width];
            }
            else { // 需要换行的时候
                NSMutableAttributedString *drawString = [self createdrawAttributedString];
                self.frameRef = [drawString prepareFrameRefWithWidth:self.width];
            ...
            //添加［查看更多］
            if (!self.closeAttString) {
                self.closeAttString = [self getColseStringWithFrameRef:self.frameRef];
            }
            CTFrameRef closeFrameRef = [self.closeAttString prepareFrameRefWithWidth:self.width];
            CTFrameDraw(closeFrameRef, context);
            
            self.moreRange = [self.closeAttString.string rangeOfString:self.attributedOpenString.string];
            self.frameRef = [self.closeAttString prepareFrameRefWithWidth:self.width];
        }
    }
}
