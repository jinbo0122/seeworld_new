...
#define VERTICAL_PADDING_HEIGHT 9.0f
...
- (id)initWithFrame:(CGRect)frame
{
  ...
  if (self) {
    ...
    self.longPressShowMenu = DEFAULT_SHOW_TAG_MENU;
    
    self.defaultSelected = -1;
    self.lastTextRect = CGRectZero;
  }
  return self;
}
  ...
  for (int i = 0; i < textArray.count; ++i) {
    ...
    [tagView updateWithString:text
           ...
           constrainedToWidth:self.frame.size.width - (self.horizontalPadding * 2)
                ...
                needLongTouch:_longPressShowMenu
     ];
    ...
    {
      tagView.button.userInteractionEnabled = NO;
    }
  }
...
- (void)touchUpInside:(id)sender
{
  ...
  if (self.showTagMenu && !_longPressShowMenu) {
...
- (void)tagViewWantsToBeDeleted:(DWTagView *)tagView {
  ...
  //mod by xiongchen
  if (_longPressShowMenu)
  {
    if ([self.tagDelegate respondsToSelector:@selector(deletedTag:)]) {
      [self.tagDelegate deletedTag:tagView.tag];
    }
  }
  else
  {
...
- (void)updateWithString:(id)text font:(UIFont*)font constrainedToWidth:(CGFloat)maxWidth padding:(CGSize)padding minimumWidth:(CGFloat)minimumWidth needLongTouch:(BOOL)longTouch
{
  ...
  if (isTextAttributedString) {
    ...
    textSize = [text sizeWithAttributes:@{NSFontAttributeName:font}
                      constrainedToSize:CGSizeMake(maxWidth, 10000)
  ...
  if (longTouch)
  {
    //增加长按效果  add by xiongchen
    UILongPressGestureRecognizer *longpressGesutre=[[UILongPressGestureRecognizer alloc]initWithTarget:self action:@selector(touchLongTime:)];
    [self addGestureRecognizer:longpressGesutre];
  }
}

- (void)touchLongTime:(UILongPressGestureRecognizer *)gestureRecognizer
{
