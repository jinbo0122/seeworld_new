...
- (void)showImage
{
  if (_photo.firstShow) { // 首次显示
    ...
    NSString *prefix = [_photo.url.absoluteString substringToIndex:4];
...
- (void)hide
{
  ...
  if ([self.photoViewDelegate respondsToSelector:@selector(viewWillHide:)]) {
    [self.photoViewDelegate viewWillHide:self];
  }
  ...
  if (_photo.srcImageView) {
    [UIView animateWithDuration:duration + 0.1 animations:^{
      ...
      // 通知代理
      if ([self.photoViewDelegate respondsToSelector:@selector(viewDidHided:)]) {
        [self.photoViewDelegate viewDidHided:self];
      }
    } completion:^(BOOL finished) {
      // 设置底部的小图片
      _photo.srcImageView.image = _photo.placeholder;
