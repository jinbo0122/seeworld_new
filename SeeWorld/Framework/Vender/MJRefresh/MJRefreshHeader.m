...
- (UIImageView *)headImageView
{
    if (!_headImageView)
    {
        UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"app_icon_pulllogo"]];
        [self addSubview:_headImageView = imageView];
        if (_allImageHiden)
        {
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        //add by xiongchen 默认不现实时间
        _updatedTimeHidden = YES;
...
- (void)layoutSubviews
{
    ...
    if (self.updatedTimeHidden) { // 显示状态
        ...
        {
            self.stateLabel.frame = CGRectMake(0, self.bounds.size.height-15, self.bounds.size.width, 10);
        }
        self.headImageView.frame = CGRectMake((self.bounds.size.width - _headImageView.image.size.width)/2, _stateLabel.frame.origin.y-_headImageView.image.size.height-5, _headImageView.image.size.width, _headImageView.image.size.height);
