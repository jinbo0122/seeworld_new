...
+ (UIImage*)thumbImage:(NSData*)imageData{
  ...
  CGSize size = CGSizeMake(90, 90*img.size.height/img.size.width);
        ...
        [group enumerateAssetsWithOptions:NSEnumerationReverse usingBlock:^(ALAsset *alAsset, NSUInteger index, BOOL *innerStop) {
          if (alAsset) {
            ...
            *stop = YES; *innerStop = YES;
            if([sender isKindOfClass:[UIImageView class]]){
              [(UIImageView* )sender setImage:latestPhoto];
            }else if ([sender isKindOfClass:[UIButton class]]){
              [(UIButton *)sender setImage:latestPhoto forState:UIControlStateNormal];
            }
          }
        }];
      }
