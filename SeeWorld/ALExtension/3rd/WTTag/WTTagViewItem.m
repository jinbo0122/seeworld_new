...
- (void)initialize
{
    ...
    self.tagImageView.frame = CGRectMake(0, 0, 21, 21);
    ...
    self.titleBgImageView.alpha = 0.8;
    
    UILabel *titleLabel = [UILabel new];
    titleLabel.textColor = [UIColor whiteColor];
    titleLabel.font = [UIFont systemFontOfSize:[self getAdaptViewScaleConstant:13.0f]];
    ...
    self.tagHoverImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0,0, 8, 8)];
    self.tagHoverImageView.center = CGPointMake((self.tagImageView.frame.size.width)/2, (self.tagImageView.frame.size.height)/2);
    self.tagHoverImageView.image = [UIImage imageNamed:@"tag_point_black"];
    self.tagHoverImageView.alpha = .3;
    [self.tagImageView addSubview:self.tagHoverImageView];
    ...
    [self.tagImageView addSubview:self.tagHoverImageView2];
    ...
    [self.tagImageView addSubview:self.whitePoint];
}
...
{
    if (nil == _leftBgImage) {
//        _leftBgImage = [[UIImage imageNamed:[WTTagViewHelper tagLeftBgImageName]] resizableImageWithCapInsets:UIEdgeInsetsMake([self getAdaptViewScaleConstant:8.0f], [self getAdaptViewScaleConstant:15.0f], [self getAdaptViewScaleConstant:2.0f], [self getAdaptViewScaleConstant:6.0f]) resizingMode:UIImageResizingModeStretch];
        
        _leftBgImage = [[UIImage imageNamed:[WTTagViewHelper tagLeftBgImageName]] stretchableImageWithLeftCapWidth:15 topCapHeight:8];
    }
    return _leftBgImage;
}
...
- (UIImage *)rightBgImage
{
    if (nil == _rightBgImage) {
        _rightBgImage = [[UIImage imageNamed:[WTTagViewHelper tagRightBgImageName]] stretchableImageWithLeftCapWidth:6 topCapHeight:8];
...
- (void)setAdaptViewScale:(CGFloat)adaptViewScale
{
    ...
    self.needUpdateContraints = YES;
    self.titleLabel.font = [UIFont systemFontOfSize:[self getAdaptViewScaleConstant:13.0f]];
...
- (void)updateConstraints
{
    if (self.needUpdateContraints) {
        ...
        self.titleBgImageView.frame = CGRectMake(self.titleBgImageView.frame.origin.x, self.titleBgImageView.frame.origin.y, self.titleBgImageView.frame.size.width, 26 * 1.5);
    }
