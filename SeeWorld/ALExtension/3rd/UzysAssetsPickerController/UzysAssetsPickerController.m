...
- (void)setupLayout
{
  ...
  NSBundle *bundle = [NSBundle bundleWithPath:[[NSBundle mainBundle] pathForResource:@"UzysAssetPickerController" ofType:@"bundle"]];
  UIImage *check = [UIImage imageWithContentsOfFile:[bundle pathForResource:@"uzysAP_ico_upload_done@2x" ofType:@"png"]];
  
  _btnDone.customImageView = [[UIImageView alloc] initWithFrame:CGRectMake((_btnDone.width-15)/2.0, (_btnDone.height-12)/2.0, 15, 12)];
  _btnDone.customImageView.image = check;
  [_btnDone addSubview:_btnDone.customImageView];
...
- (void)finishPickingAssetsWithPreview:(BOOL)needPreview
{
  ...
  if([assets count]>0)
  {
    ...
    [[[UIAlertView alloc] initWithTitle:@"請選擇一張照片"
                                message:nil
