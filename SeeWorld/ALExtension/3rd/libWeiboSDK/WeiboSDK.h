...
/**
 ...
 此方法生成对象被[WeiboSDK sendRequest:]会唤起微博客户端的發佈器进行分享，如果未安装微博客户端或客户端版本太低
...
/**
 ...
 当用户没有安装微博客户端或微博客户端过低无法支持通过客户端內分享的时候会自动唤起SDK內微博發佈器
 
 @param message 需要发送给微博的消息对象
 @param authRequest 授权相关信息,与access_token二者至少有一个不为空,当access_token为空并且需要弹出SDK內發佈器时会通过此信息先进行授权后再分享
