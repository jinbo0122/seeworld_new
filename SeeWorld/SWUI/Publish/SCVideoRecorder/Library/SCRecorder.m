...
// 使用VideoCore VCPreviewView 进行预览摄像头 可以看到美白后结果
#define USE_VIDEOCORE_PREVIEW  0

#if USE_VIDEOCORE_PREVIEW
...
#if USE_VIDEOCORE_PREVIEW
    // 添加美白预览功能
    VCPreviewView*  _vcPreview;
    PDGLVideoBeautyRender* _beautyRender;
...
#if USE_VIDEOCORE_PREVIEW
        _vcPreview = [[VCPreviewView alloc] init];
        _beautyRender = [[PDGLVideoBeautyRender alloc] init];
#endif
    }
            ...
            if (recordSession.recordSegmentReady) {
                ...
                if (recordSession.isUsingMovieFileOutput) {
                    ...
                    [recordSession endSegmentWithInfo:info completionHandler:^(SCRecordSessionSegment *segment, NSError *error) {
                        ...
                        NSLog(@"segment:%@ url:%@",segment, segment.url);
                        if (completionHandler != nil) {
                            completionHandler();
                        }
                    }];
...
// 使用 CVPixelBufferRef 接口进行渲染
- (void)appendVideoSampleBuffer2:(CVPixelBufferRef)sampleBufferImage PTS:(CMTime) PTS
                 toRecordSession:(SCRecordSession *)recordSession
    ...
    if (filterGroup == nil && transformFilter == nil) {
        [recordSession appendVideoPixelBuffer:sampleBufferImage atTime:PTS duration:duration completion:completion];
        return;
    }
    ...
    CIImage *image = [CIImage imageWithCVPixelBuffer:sampleBufferImage];
    CFTimeInterval seconds = CMTimeGetSeconds(PTS);
    ...
    [recordSession appendVideoPixelBuffer:pixelBuffer atTime:PTS duration:duration completion:^(BOOL success) {
        CVPixelBufferUnlockBaseAddress(pixelBuffer, 0);
...
#if USE_VIDEOCORE_PREVIEW
        // 美白流程
        CVPixelBufferRef drawPixelBuffer = nil;
        if (true) {
            CVPixelBufferRef ref = CMSampleBufferGetImageBuffer(sampleBuffer);
            
            int frame_width = (CGFloat)CVPixelBufferGetWidth(ref);
            int frame_height = (CGFloat)CVPixelBufferGetHeight(ref);
            
            LyGLFrameBuffer* framebuff = [_beautyRender fetchFramebufferForSize:CGSizeMake(frame_width, frame_height)];
            [_beautyRender offScreenBeautyRender:ref atFramebuffer:framebuff];
            drawPixelBuffer = framebuff.renderTarget;
        }
        
        // 预览界面渲染
        if (drawPixelBuffer && _vcPreview) {
            [_vcPreview drawFrame:drawPixelBuffer];
        }
#endif
...
//                NSLog(@"APPENDING");
                void(^myBlock)(BOOL success)  = ^(BOOL success){
...
#if USE_VIDEOCORE_PREVIEW
                [self appendVideoSampleBuffer2:drawPixelBuffer
                                           PTS:CMSampleBufferGetPresentationTimeStamp(sampleBuffer)
                               toRecordSession:recordSession
                                      duration:duration connection:connection completion:myBlock];
                
                
#else
                [self appendVideoSampleBuffer:sampleBuffer toRecordSession:recordSession duration:duration connection:connection completion:myBlock];
            ...
            if (newInput != nil) {
                if ([_captureSession canAddInput:newInput]) {
                    ...
                    if ([newInput.device hasMediaType:AVMediaTypeVideo]) {
                        ...
                        if (newInput.device.position == AVCaptureDevicePositionFront) {
//                            [_beautyRender isFlipXPosition:1];
                        } else {
//                            [_beautyRender isFlipXPosition:0];
                        }
#endif
...
#if USE_VIDEOCORE_PREVIEW
  _vcPreview.frame = _previewView.frame;
#endif
}
...
- (void)setPreviewView:(UIView *)previewView {
    ...
    if (_previewView != nil) {
#if USE_VIDEOCORE_PREVIEW
        [_previewView insertSubview:_vcPreview atIndex:0];
#else
