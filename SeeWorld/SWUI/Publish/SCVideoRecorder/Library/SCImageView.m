...
- (void)glkView:(GLKView *)view drawInRect:(CGRect)rect {
    @autoreleasepool {
        ...
        if (image != nil) {
            ...
            if (_isAdjustRenderRect) {
                // FIXME: 暂时修改渲染矩形达到保持分辨率目的
                if (image.extent.size.width < rect.size.width) {
                    rect.origin.x = (rect.size.width - image.extent.size.width) / 2.0f;
                    rect.size.width = image.extent.size.width;
                }
                
                if (image.extent.size.height < rect.size.height) {
                  rect.origin.y = (rect.size.height - image.extent.size.height) / 2.0f;
