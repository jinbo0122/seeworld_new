...
// PRIVATE DEFINITION
/////////////////////
// SCPlayer 添加AVAudioSessionInterruptionNotification 中断处理
@interface SCPlayer() <AVPlayerItemOutputPullDelegate, AVPlayerItemOutputPushDelegate> {
    ...
    BOOL _isBackGround;
    NSString* _audioSessionCategory;  // 保存之前AVAudioSession 类型
}
...
- (void)dealloc {
    ...
    // 还原AVAudioSession 音频类型
    if (_audioSessionCategory) {
        NSError *error = nil;
        BOOL success = NO;
        success = [[AVAudioSession sharedInstance] setCategory:_audioSessionCategory error:&error];
        if (!success) {
            NSLog(@"%@ Error setting category: %@", NSStringFromSelector(_cmd), [error localizedDescription]);
        }
    }
...
- (void)setupVideoOutputToItem:(AVPlayerItem *)item {
    if (_displayLink != nil && item != nil && _videoOutput == nil && item.status == AVPlayerItemStatusReadyToPlay) {
      ...
      if (CGAffineTransformEqualToTransform(_rendererTransform, CGAffineTransformMake(0, 0, 0, 0, 0, 0))) {
...
- (void)sessionInterrupted:(NSNotification *)notification {
    ...
    if (interruptionState == AVAudioSessionInterruptionTypeEnded) {
        if (!_isBackGround) {
            [self play];
